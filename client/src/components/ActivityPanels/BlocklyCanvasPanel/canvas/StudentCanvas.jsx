...
export default function StudentCanvas({ activity }) {
  ...
  //Uses a simple string comparator to compare grade templates
  const handleAutograde = async () => {
      await handleManualSave()

      let activityTrue = await getActivity(activity.id)
      if(activityTrue.data.autograde_enabled){
        let hits = 0;
        let length = 0;

        let xml = window.Blockly.Xml.workspaceToDom(workspaceRef.current);
        let saveString = window.Blockly.Xml.domToText(xml);

        const gradeTemplate = activityTrue.data.grade_template
