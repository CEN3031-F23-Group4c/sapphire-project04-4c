...
import { 
  ...
  handleCreatorSaveGradeActivity
 } from '../../Utils/helpers';
...
export default function MentorCanvas({ activity, isSandbox, setActivity,  isMentorActivity, isGradeActivity }) {
  ...
  useEffect(() => {
    ...
    const setUp = async () => {
      ...
      if (!workspaceRef.current && activity && Object.keys(activity).length !== 0) {
        ...
        // }
        let xml = null
        if(isMentorActivity && !isGradeActivity){
         xml = window.Blockly.Xml.textToDom(activity.activity_template)
        }
        else if (!isMentorActivity && isGradeActivity){
          xml =window.Blockly.Xml.textToDom(activity.grade_template)
        }
        else{
          xml = window.Blockly.Xml.textToDom(activity.template);
        }
      window.Blockly.Xml.domToWorkspace(xml, workspaceRef.current);
  ...
  const handleCreatorSave = async () => {
    ...
    if (!isSandbox && !isMentorActivity && !isGradeActivity) {
    ...
    } else if (!isSandbox && isMentorActivity && !isGradeActivity) {
      // Save activity template
    ...
    } else if (!isSandbox && isGradeActivity && !isMentorActivity) {
      // Save grade template
      const res = await handleCreatorSaveGradeActivity(activity.id, workspaceRef);
      if (res.err) {
        message.error(res.err);
      } else {
