...
export const handleCreatorSaveGradeActivity = async (activityId, workspaceRef) => {
  //Uses the same xml features in order to save grade template
  let xml = window.Blockly.Xml.workspaceToDom(workspaceRef.current);
  let xml_text = window.Blockly.Xml.domToText(xml);

  return await updateGradeActivityTemplate(activityId, xml_text);
};
