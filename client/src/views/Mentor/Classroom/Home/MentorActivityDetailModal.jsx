...
const MentorActivityDetailModal = ({
  ...
  const [grade_template, setGradeTemplate] = useState("")
  const [enableAutograde, setAutograde] = useState(false)
  ...
  useEffect(() => {
    const showActivityDetailsModal = async () => {
      ...
      setGradeTemplate(response.data.grade_template)
      setStandardS(response.data.StandardS)
      setImages(response.data.images)
      setLink(response.data.link)
      setAutograde(response.data.autograde_enabled)
      setLinkError(false)
  ...
  const handleViewActivityGradeTemplate = async activity => {
    const allToolBoxRes = await getActivityToolboxAll()
  ...
  const handleAutomaticGrading = async activity =>{
    if(enableAutograde){
      setAutograde(false)
    }
    else{
  ...
  const handleSave = async () => {
    ...
    if (res.err) {
      ...
      if (submitButton === 0) {
        ...
        setActivityDetailsVisible(false)
        handleViewActivityGradeTemplate(res.data)
      }
    }
  }
  const showModal = () => {
...
<g
    ...
    >
      ...
      >
        ...
        <h4 id="subtitle">Automatic Grading</h4>
        <Form.Item>
          <Checkbox onChange={() => handleAutomaticGrading()}
        ...
        >
          ...
          <button id="save--set-grade-btn" onClick={() => setSubmitButton(3)}>
            Edit Grade 
            <br />
            Template
          </button>
