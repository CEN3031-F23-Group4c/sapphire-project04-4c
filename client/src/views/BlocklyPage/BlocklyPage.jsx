...
export default function BlocklyPage({ isSandbox }) {
  ...
  const [isGradeActivity, setGradeActivity] = useState(false);
  ...
  useEffect(() => {
    const setup = async () => {
      ...
      // else show toolbox based on the activity we are viewing
        ...
        if(localStorage.getItem("isGradeActivity")){
          setGradeActivity(true)
          localStorage.removeItem("isGradeActivity")
        }
  ...
  return (
    <div className="container nav-padding">
      ...
      <div className="flex flex-row">
        ...
        isGradeActivity={isGradeActivity} />
      </div>
    </div>
  )
}
