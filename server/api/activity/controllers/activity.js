...
module.exports = {
  ...
  async update(ctx) {
    ...
    const updatedActivity = await strapi.services.activity.update(
      { id },
      { description, images, StandardS, link, learning_components: activityComponents, autograde_enabled: enableAutograde }
    );
  ...
  async toolbox(ctx) {
   ...
   async gradeTemplateUpdate(ctx) {
    ...
    // update template
    activity.grade_template = ctx.request.body.grade_template;
